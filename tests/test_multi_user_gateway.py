...
def test_new_user_container_uses_resource_and_log_limits(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(gateway.UserGateway, "wait_for_user_http", lambda _self, _user: None)
    monkeypatch.setenv(
        gateway.DEFAULT_DISCORD_WEBHOOK_ENV,
        "https://discord.com/api/webhooks/123/token",
    )
    monkeypatch.setenv(
        gateway.DEFAULT_DISCORD_CHANNEL_ENV,
        "123456789012345678",
    )
    ...
    assert gateway.DEFAULT_DISCORD_CHANNEL_ENV in docker_run
    assert command_envs[-1]["TOSS_CLIENT_ID"] == "id"
    assert command_envs[-1]["TOSS_CLIENT_SECRET"] == "secret"
    assert command_envs[-1][gateway.DEFAULT_DISCORD_WEBHOOK_ENV] == "https://discord.com/api/webhooks/123/token"
    assert command_envs[-1][gateway.DEFAULT_DISCORD_CHANNEL_ENV] == "123456789012345678"
